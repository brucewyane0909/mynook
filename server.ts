...
const getChapterContentDeclaration: FunctionDeclaration = {
...
const proposeManuscriptEditDeclaration: FunctionDeclaration = {
...
const proposeCreateChapterDeclaration: FunctionDeclaration = {
...
async function generateStreamWithFallback(ai: GoogleGenAI, requestOptions: any) {
...
const SYSTEM_INSTRUCTION = `You are MYNOOK AI, the intelligent, empathetic literary writing assistant and novel editor built inside the MYNOOK book studio.
...
app.post(['/api/ai', '/api/ai/chat'], async (req, res) => {
  try {
    const {
      messages = [],
      bookContext = {},
    ...
    console.log(`[MYNOOK AI] AI request started (stream: ${stream}, action: ${action || 'none'}, messages count: ${messages?.length || 0})`);
    ...
    if (bookContext.currentChapterContent) {
      // Clean HTML tags for model reading efficiency
    ...
    // Include any pre-fetched chapterLookup if passed
    ...
    // If an action was specified (e.g. quick action), tailor the last user message
    if (action) {
    ...
    if (stream) {
      ...
      res.on('close', () => {
        if (!res.writableEnded) {
          isClosed = true;
          console.log('[MYNOOK AI] Client disconnected connection prematurely');
        }
      });

      try {
        const { stream: streamResponse, modelUsed } = await generateStreamWithFallback(ai, {
          contents,
          config: {
        ...
        console.log(`[MYNOOK AI] Active stream established using model: ${modelUsed}`);
        ...
        for await (const chunk of streamResponse) {
          ...
          chunkCount++;
          if (chunkCount === 1) {
            console.log('[MYNOOK AI] Gemini response received (first chunk streamed)');
          }
        ...
        if (!isClosed) {
          console.log(`[MYNOOK AI] Gemini response completed successfully (${fullText.length} chars)`);
          res.write(`data: ${JSON.stringify({ type: 'done', fullText })}\n\n`);
          res.end();
        }
      } catch (streamError: any) {
        console.error('[MYNOOK AI] Gemini request failed:', streamError?.message || streamError);
        if (!isClosed) {
          res.write(
            `data: ${JSON.stringify({
              type: 'error',
